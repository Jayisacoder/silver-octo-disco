...
### 5. Install the required commit gate
...
The second command must print `.githooks`. Repeat setup for every new clone. The hook runs **all tests and the production build before each commit** and blocks the commit if any command fails. Configure your real test commands before your first project commit; the starter placeholders intentionally fail. See [Testing and commit requirements](docs/TESTING.md).
...
|---|---|---|
| 1. Set up | Clone [organizational `main`](https://github.com/LaunchPadPhilly/claude-code-agent-project), create your GitHub fork, connect it as `origin`, and create your submission branch. | Your fork URL, branch, and installed commit hook; see [submission instructions](docs/SUBMISSION.md). |
...
| 5. Build | Author your agents and Skill, implement your feature, and configure real unit, full-suite, and build commands. | Working application, six required agent roles, one reusable Skill, and two recorded delegation examples. |
...
The placeholder `test:unit`, `test`, and `build` commands intentionally fail until you replace them with real project checks. `test:all` runs the repository gate tests, your unit tests, and your full test command; include every additional project suite in `npm test`. Keep your designs learner-authored; the starter supplies requirements, not implementations.
...
- [ ] Run `npm run test:all` and `npm run build`, fix failures, and record the actual results.
- [ ] Install the commit hook and confirm no code commits bypass the passing-test requirement.
...
Confirm the hook is installed (`git config --get core.hooksPath` prints `.githooks`). Stage all intended changes before committing: the gate rejects unstaged edits and untracked files so tests validate the version being committed. `git commit` runs `npm run test:all` and `npm run build`; fix failures and retry. Do not bypass the hook or disable failing tests.
